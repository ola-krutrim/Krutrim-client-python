...
async def main() -> None:
    async with AsyncKrutrimClient() as client:
        async with await client.sandbox.create(
            ...
            await sandbox.files.write(f"{WORKSPACE}/program.py", agent_program())
            await sandbox.files.write(f"{WORKSPACE}/input.json", json.dumps({"value": 21}))
            ...
            raw_output = await sandbox.files.read(f"{WORKSPACE}/output.json")
